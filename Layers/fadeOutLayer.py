...
class FadeOutLayer(nn.Module):
    samplingBit=16
    def __init__(self, seconds):
        super(FadeOutLayer, self).__init__()
    ...
    def forward(self, input:tf.Tensor)->tf.Tensor:
        ...
        filt[-l:] = tf.arange(l-1, -1, -1, dtype=tf.float32)/l
        out = input.mul(filt).mul(filt)
        return out

        
layer=FadeOutLayer(30)
...
if __name__ == "__main__":
    ...
    #exit()
    writer=wave.open(f"{DIR}/../Data/test-FadeOut.wav","wb")
